...
try {
  ...
  errors.push(`config.json 解析失败: ${error instanceof Error ? error.message : error}`);
}

const relations = (config.initialRelations as { characterId: string; characterName: string }[] | undefined) ?? [];
const characterIds = relations.map((r) => r.characterId);

check(typeof config.title === "string" && config.title !== "", "config.title 缺失");
check(
  typeof config.protagonistName === "string",
  "config.protagonistName 缺失",
);
check(
...
const emoji = (config.characterEmoji as Record<string, string> | undefined) ?? {};
