...
export function listSaves(): SaveMeta[] {
  const rows = getDb().query('SELECT id, data FROM saves').all() as {
    id: string
    data: string
  }[]
  return rows
...
export function deleteSave(id: string): boolean {
  ...
  const db = getDb()
  const result = db.query('DELETE FROM saves WHERE id = ?').run(validId)
...
export function searchMemories(
  ...
  const rows = db
    ...
    .all(match, validId, maxSeqRow.max - excludeRecent, limit) as {
    seq: number
    role: string
    content: string
    day: number | null
  ...
  return rows.map((row) => ({
    ...
    chapter: row.chapter,
    content:
      row.role === 'assistant' ? extractNarration(row.content) : row.content
  }))
}
...
export function popLastTurn(saveId: string): Promise<boolean> {
  ...
  return enqueueWrite(validId, () => {
    ...
    const rows = db
      .query(
        'SELECT seq, role FROM conversations WHERE save_id = ? ORDER BY seq DESC LIMIT 2'
      )
      .all(validId) as { seq: number; role: string }[]
    if (rows.length < 2) return false
    if (rows[0].role !== 'assistant' || rows[1].role !== 'user') return false
    db.query(
      'DELETE FROM conversations WHERE save_id = ? AND seq IN (?, ?)'
