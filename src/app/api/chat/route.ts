...
function applyStateChanges(
  ...
  if (changes.chapter) {
    ...
    if (
      !chapters ||
      chapters.length === 0 ||
      chapters.includes(changes.chapter)
    ) {
        ...
        for await (const chunk of streamChat(messages)) {
          ...
          if (parsed && !updateEmitted) {
            ...
            if (parsed.stateChanges) {
              payload.stateChanges = applyStateChanges(
                save,
                parsed.stateChanges
              )
            }
        ...
        if (parsed && !isRegenerate) {
          ...
          if (parsed.newItems) {
            ...
            for (const newItem of parsed.newItems) {
              const existing = currentItems.find((i) => i.itemId === newItem.id)
              if (existing) {
                existing.quantity += 1
              } else {
                currentItems.push({
