...
- **阶段 2（关键词检索，推荐先做，S-M）**：SQLite **FTS5 全文索引**直接建在 conversations 表上——语料已就位（每回合叙述都在库里），检索查询用当前状态块的实体（人名/地点/物品）+ 玩家输入关键词，top-3 注入【相关回忆】块。**零嵌入模型、零新增依赖**，SQLite 内置 FTS5。游戏回忆需求大多是实体锚定（"莉娅上次说了什么""吊坠哪来的"），关键词检索能覆盖大头
