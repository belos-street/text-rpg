...
> 前置文档：[bug-fix-todo.md](./bug-fix-todo.md)（43 项审查问题已全部关账——完成或按决策不做）
...
| ✅ 3 | 长期记忆检索（RAG）——阶段 1 真摘要 + 阶段 2 FTS5 检索（2026-09-12） | L | 已完成（阶段 3 向量按需再做） |
| 4 | 消息列表虚拟化 | M | 单日消息 >200 或滚动明显卡顿 |
| ✅ 6 | 调试面板最小版（D4）（2026-09-12） | M | 已完成 |
| ✅ 7 | 真 LLM 摘要（M3）（2026-09-12） | M | 已完成 |
| ✅ 8 | 格式化工具链（#22）（2026-09-12） | S | 已完成 |
...
- **阶段 2（关键词检索）** ✅ 已完成（2026-09-12）：FTS5 虚拟表 `fts_memories` + 中文 bigram 分词（[utils.ts](src/lib/utils.ts) `toBigrams`，2 字角色名可精确命中）；检索查询 = 玩家输入 + 高好感角色名 + 当前地点，top-3 注入【相关回忆】块；append/popLastTurn/deleteSave 同步维护索引，存量数据启动回填。零嵌入模型、零新增运行时依赖
- **阶段 3（向量 RAG，按需再做，M）**：语义回忆（"那个下雨天躲雨的地方"这类模糊查询）才需要向量。要点：
  - **嵌入模型选型**：`nomic-embed-text-v1.5` 中文效果一般；中文游戏应选 **bge-small-zh-v1.5**（24M，极小）或 **bge-m3**（566M，多语言，LM Studio 可直接下载）
  - 本地同时跑嵌入模型**完全可行**：24M-566M 对比 26B 主模型资源可忽略，LM Studio 可常驻或 JIT（嵌入调用每回合一次，约 50-200ms）
  - 流水线：append 时异步嵌入叙述 → 存 `(save_id, seq, embedding BLOB)` → 每回合用"玩家输入+当前状态"做查询 → 余弦 top-k → 注入提示词
...
> 用户决策（2026-09-12）：不做，单故事包已满足需求（详见文末「有意不做」）。以下为原方案存档。
...
- [x] **#22 格式化工具链** ✅（2026-09-12）：oxfmt 配置（单引号/无分号/无尾逗号/2 空格/80 列，对齐 AGENTS.md）+ `format`/`format:check` scripts，单独一次"仅格式化"提交（06a8fc4）；组件文件名统一 kebab-case（用户决策），单独重命名提交（684c941）
...
| 多故事包（D2） | 用户决策（2026-09-12）：不做，单故事包已满足需求 |
